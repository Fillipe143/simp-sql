...
random_color :: proc() -> rl.Color {
    return rl.ColorFromHSV(
        f32(rl.GetRandomValue(0, 360)),
        f32(rl.GetRandomValue(42, 98)) / 100,
        f32(rl.GetRandomValue(40, 90)) / 100,
    )
}
...
draw_connections :: proc(node: ^Node) {
    for i in 0 ..< len(node.childrens) {
        child := &node.childrens[i]
        rl.DrawLineEx(rl.Vector2{node.x, node.y}, rl.Vector2{child.x, child.y}, 3.0, rl.DARKGRAY)
...
draw_nodes :: proc(ctx: ^Context, node: ^Node) {
    ...
    text_pos := rl.Vector2 {
        node.x - text_size.x / 2, 
        node.y - text_size.y / 2,
    }
