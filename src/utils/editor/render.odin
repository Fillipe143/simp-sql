...
is_sql_keyword :: proc(word: string) -> bool {
    KEYWORDS := []string{
        ...
        "TABLE", "INTO", "VALUES", "AND", "OR", "NOT", "NULL", "JOIN", "LEFT",
        "RIGHT", "INNER", "GROUP", "BY", "ORDER", "HAVING", "LIMIT", "AS", "IN", "ON",
        "BETWEEN", "LIKE", "IS", "ANY", "ALL", "EXISTS", "DISTINCT", "DATABASE",
    }
...
get_gutter_width :: proc(ctx: ^Context) -> f32 {
    ...
    text_size := rl.MeasureTextEx(ctx.font, strings.clone_to_cstring(str, context.temp_allocator), font_size, 2)
...
update_scroll :: proc(ctx: ^Context) {
    ...
    measure_x := rl.MeasureTextEx(ctx.font, strings.clone_to_cstring(text_before, context.temp_allocator), font_size, 2).x
...
render_status_bar :: proc(ctx: ^Context) {
    ...
    case .NORMAL: accent_color = {255, 222, 33, 255}
    case .INSERT: accent_color = {0, 150, 255, 255}
    }
    ...
    rl.DrawTextEx(ctx.sfont, pos_cstr, {f32(ctx.x + ctx.w) - pos_width - 15, bar_y + 10}, font_size, 2, text_color)
}
...
render_lines :: proc(ctx: ^Context, start, end: i32) {
    ...
    for i in start ..< end {
        ...
        for token in tokens {
            token_cstr := strings.clone_to_cstring(token.text, context.temp_allocator)
            rl.DrawTextEx(ctx.font, token_cstr, {current_x, y_pos}, font_size, spacing, token.color)
...
render_cursor :: proc(ctx: ^Context) {
    ...
    if ctx.keyboard.mode == .NORMAL {
        ...
        draw_pos := rl.Vector2{
            1 + cursor_pos.x - (padding_x / 2),
            cursor_pos.y - (padding_y / 2),
        }
        
        draw_size := rl.Vector2{
            char_width + padding_x,
            font_size + padding_y,
        }
