...
valid_column :: proc(t_name: string, col: string) -> bool {
    t_lower := strings.to_lower(t_name, context.temp_allocator)
    c_lower := strings.to_lower(col, context.temp_allocator)

    switch t_lower {
    case "categoria":
        return c_lower == "idcategoria" || c_lower == "descricao"
    case "produto":
        return(
            c_lower == "idproduto" ||
            c_lower == "nome" ||
            c_lower == "descricao" ||
            c_lower == "preco" ||
            c_lower == "quantestoque" ||
            c_lower == "categoria_idcategoria" \
        )
    case "tipocliente":
        return c_lower == "idtipocliente" || c_lower == "descricao"
    case "cliente":
        return(
            c_lower == "idcliente" ||
            c_lower == "nome" ||
            ...
            c_lower == "nascimento" ||
            c_lower == "senha" ||
            c_lower == "tipocliente_idtipocliente" ||
            c_lower == "dataregistro" \
        )
    case "tipoendereco":
        return c_lower == "idtipoendereco" || c_lower == "descricao"
    case "endereco":
        return(
            c_lower == "idendereco" ||
            c_lower == "enderecopadrao" ||
            c_lower == "logradouro" ||
            c_lower == "numero" ||
            c_lower == "complemento" ||
            c_lower == "bairro" ||
            ...
            c_lower == "uf" ||
            c_lower == "cep" ||
            c_lower == "tipoendereco_idtipoendereco" ||
            c_lower == "cliente_idcliente" \
        )
    case "telefone":
        return c_lower == "numero" || c_lower == "cliente_idcliente"
    case "status":
        return c_lower == "idstatus" || c_lower == "descricao"
    case "pedido":
        return(
            c_lower == "idpedido" ||
            c_lower == "status_idstatus" ||
            c_lower == "datapedido" ||
            c_lower == "valortotalpedido" ||
            c_lower == "cliente_idcliente" \
        )
    case "pedido_has_produto":
        return(
            c_lower == "idpedidoproduto" ||
            c_lower == "pedido_idpedido" ||
            c_lower == "produto_idproduto" ||
            c_lower == "quantidade" ||
            c_lower == "precounitario" \
        )
    }
    return false
}
...
column_belongs_to_table :: proc(col_expr: string, table_label: string) -> bool {
    ...
    if c_alias != "" && t_alias != "" {
        if !strings.equal_fold(c_alias, t_alias) do return false
    }
    ...
    for t in tables_nodes {
        ...
        for fc in final_cols {
            if column_belongs_to_table(fc, t.label) {
                found := false
                for c in my_cols {if strings.equal_fold(c, fc) do found = true}
        ...
        for j in joins {
            ...
            if len(parts) == 2 {
                ...
                if column_belongs_to_table(p0, t.label) {
                    found := false
                    for c in my_cols {if strings.equal_fold(c, p0) do found = true}
                    if !found do append(&my_cols, p0)
                }
                if column_belongs_to_table(p1, t.label) {
                    found := false
                    for c in my_cols {if strings.equal_fold(c, p1) do found = true}
        ...
        if len(branches) > 0 {
            ...
            for fc in final_cols {
                if branch_provides_column(current_tree, fc) {
                    found := false
                    for c in needed_cols {if strings.equal_fold(c, fc) do found = true}
            ...
            for j in joins {
                ...
                if len(parts) == 2 {
                    ...
                    if branch_provides_column(current_tree, p0) {
                        found := false
                        for c in needed_cols {if strings.equal_fold(c, p0) do found = true}
                        if !found do append(&needed_cols, p0)
                    }
                    if branch_provides_column(current_tree, p1) {
                        found := false
                        for c in needed_cols {if strings.equal_fold(c, p1) do found = true}
...
validate_column_in_query :: proc(query: sql.Query, col_alias: string, col_name: string) -> string {
    ...
    check_table :: proc(
        ...
        if c_alias != "" {
            if strings.equal_fold(c_alias, t_alias) || strings.equal_fold(c_alias, t_name) {
                valid_alias^ = true
